...
if __name__ == '__main__':
    dev_file_path = "../../../../public_data/augment/RE/redocred/redocred_dev.json"
    data = json.load(open(dev_file_path))
    sample_one = random.sample(data,1)[0]
    print(sample_one)
    fact_list = "\n".join(str(fact) for fact in sample_one['fact_list'])
    print(fact_list)
    one_example = "One example like:\n" \
                  f"Given a the passage : \"{sample_one['passage']}\".\n" \
                  "Derive all the triplet facts from the passage according to the given relations.\n" \
                  f"{fact_list}"
    test_file_path = "../../../../public_data/augment/RE/redocred/redocred_test.json"

    save_path = "./result_no_desc_no_given_few_shot/joint_sentence_fact"
