...
relations_description = json.load(open("../../../public_data/relations_desc/ori_relation_description.json"))
...
def inference(test_model_type, model, tokenizer, text):
    if test_model_type == "chatglm":
        outputs, history = model.chat(tokenizer, text, do_sample=True, temperature=0.8, history=[])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--relation_part", type=int, required=False, help="dived relation", default=10)
    args = parser.parse_args()
    args.data = get_test_data(args)
    if args.data:
        args.model, args.tokenizer = get_model(args.test_model_type, args.local_rank, args.model_path)
        if args.mode == "joint_sentence_one_relation_fact":
