...
model_paths["chatglm"]="/workspace/xll/checkpoints/THUDM6B2/"

DATA_PATH="/workspace/xll/analysis_kg/public_data/augment/RE/redocred/redocred_test.json"
modes=("joint_sentence_relation_fact" "joint_sentence_one_relation_fact" "joint_sentence_one_relation_subject_fact")
...
for model in "${model_order[@]}"; do
  ...
  for mode in "${modes[@]}"; do
    ...
    run_cmd="/workspace/xll/Anaconda3/envs/chatglm/bin/deepspeed --master_port 12347 --include localhost:1,2,3,4,5,6,7 ${ARGS}"
...
#          --data_path ${DATA_PATH} \
#          --save_path ${EVAL_SAVE_PATH}"
#    run_cmd="/workspace/xll/Anaconda3/envs/chatglm/bin/deepspeed --master_port 12347 --include localhost:4 ${ARGS}"
#    eval ${run_cmd} 2>&1 | tee -a "${EVAL_SAVE_PATH}/log.log"
#  done
#done
#
